...
def update_record_info(inspire_id, send_email=False):
    ...
    if hep_submission.overall_status == 'finished' or hep_submission.version > 1:
        # For large submissions, batch the indexing to reduce memory usage
        if len(record_ids) > 100:
            log.info('Large submission detected ({} records), using batched indexing'.format(len(record_ids)))
            batch_size = 100
            for i in range(0, len(record_ids), batch_size):
                batch_record_ids = record_ids[i:i+batch_size]
                index_record_ids(batch_record_ids)  # index for OpenSearch
        else:
...
def _get_url(page, specified_time, on_or_since):
    ...
    url = ('https://inspirehep.net/api/literature?sort=mostrecent&size={}&page={}&fields=control_number'.format(size, page) +
           '&q=external_system_identifiers.schema%3AHEPData%20and%20du%3A{}'.format(specified_time.strftime("%Y-%m-%d")) +
           ('' if on_or_since == 'on' else '%2B'))
    return url
