...
from hepdata.config import SIMPLEANALYSIS_FILE_TYPE
from hepdata.cli import fix
from hepdata.ext.opensearch.api import reindex_batch
from hepdata.modules.records.utils.common import is_simpleanalysis
...
def add_simpleanalysis_analyses(batch_size, synchronous=False):
    ...
    while count < total:
        ...
        if synchronous:
            _add_simpleanalysis_analyses_batch(batch_ids)
        else:
            log.info('Sending batch of IDs {0} to {1} to celery'.format(batch_ids[0], batch_ids[-1]))
            dynamic_tasks.delay('_add_simpleanalysis_analyses_batch', 'add_simpleanalysis_analyses', batch_ids)
        count += batch_size


@shared_task
def _add_simpleanalysis_analyses_batch(ids):
    log.info(f"Checking for SimpleAnalysis resources in submission ids {ids}")
    recids_to_reindex = []
    for id in ids:
        ...
        if hepsubmission:
            for resource in hepsubmission.resources:
                if resource.file_type != SIMPLEANALYSIS_FILE_TYPE and \
                    is_simpleanalysis(resource.file_description):
                        log.info(f"Found SimpleAnalysis for resource {resource.file_location}")
                        # Update resource to have type SimpleAnalysis
                        resource.file_type = SIMPLEANALYSIS_FILE_TYPE
