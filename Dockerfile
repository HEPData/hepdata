...
RUN bash -c "set -x; [[ ${APP_ENVIRONMENT:-prod} = local-web ]] && (cd /usr/local/var && wget https://saucelabs.com/downloads/sauce-connect/5.5.0/sauce-connect-5.5.0_${SAUCE_OS:-linux.x86_64}.tar.gz && \
  tar -xvf sauce-connect-5.5.0_${SAUCE_OS:-linux.x86_64}.tar.gz) || echo 'Not installing SC on prod or worker build'"
